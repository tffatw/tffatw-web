...
export default function AnalyticsDashboard({ analytics }) {
  const isLive = analytics?.available
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#666666] mt-1">
          資料來源：{isLive ? 'Cloudflare Zone Analytics（真實數據）' : 'Cloudflare Web Analytics（示意數據）'}
        </p>
      </div>
      ...
      {isLive ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard title="今日造訪" value={analytics.todayVisits} subtitle="次" trend={analytics.trend} />
            <StatCard
              title="今日不重複訪客"
              value={analytics.todayUniques ?? '—'}
              subtitle="人（依 IP 估算）"
            />
            <StatCard title="本週累計造訪" value={analytics.weekTotal} subtitle="次" />
            <StatCard title="7 天日均造訪" value={analytics.dailyAverage} subtitle="次" />
          </div>

          <TrendChart data={analytics.weeklyData} />

          <div className="bg-[#EBE4D8]/30 border border-[#EBE4D8] rounded-xl p-4 text-sm text-[#666666]">
            <strong className="text-[#333333]">注意：</strong>
            以上數據來自 Cloudflare Zone Analytics（GraphQL API）。跳出率、平均停留時間、新／舊訪客等 RUM
            指標沒有 API 可串接，請至 Cloudflare Dashboard → Web Analytics 查看。
          </div>
        </>
      ) : (
        <>
          ...
          <TrendChart data={demoWeeklyData} />
          ...
          <div className="bg-[#EBE4D8]/30 border border-[#EBE4D8] rounded-xl p-4 text-sm text-[#666666]">
            <strong className="text-[#333333]">注意：</strong>
            以上為示意數據。{analytics?.error ? `串接 Cloudflare API 失敗（${analytics.error}）。` : ''}
            若要顯示真實資料，請設定{' '}
            <code className="mx-1 px-1.5 py-0.5 bg-white rounded text-xs">CLOUDFLARE_API_TOKEN</code>
            與{' '}
            <code className="mx-1 px-1.5 py-0.5 bg-white rounded text-xs">CLOUDFLARE_ZONE_ID</code>
            環境變數（需具備 Zone Analytics: Read 權限，且網域需透過 Cloudflare 代理）。
          </div>
        </>
      )}
    </div>
