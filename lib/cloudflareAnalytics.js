...
export async function getWeeklyAnalytics() {
  ...
  try {
    ...
    const weekTotal = rows.reduce((total, row) => total + row.sum.visits, 0)

    const todayVisits = todayRow?.sum.visits ?? 0
    const trend = yesterdayRow?.sum.visits
      ? Math.round(((todayVisits - yesterdayRow.sum.visits) / yesterdayRow.sum.visits) * 1000) / 10
      : undefined
